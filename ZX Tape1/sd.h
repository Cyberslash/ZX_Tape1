...
const char Text_SD_No_SPI_Up[] PROGMEM =       "The memory card is not \0";
const char Text_SD_No_SPI_Down[] PROGMEM =     "supports SPI\0";
const char Text_SD_No_Response[] PROGMEM =     "Map shut up!   \0";
const char Text_SD_Size_Error_Up[] PROGMEM =   "Capacity of SD card  \0";
const char Text_SD_Size_Error_Down[] PROGMEM = "not defined!   \0";
const char Text_SD_Size[] PROGMEM =            "Capacity of SD card  \0";
//----------------------------------------------------------------------------------------------------
//прототипы функций
//----------------------------------------------------------------------------------------------------
void SD_Init(void);//инициализаци€ карты пам€ти

unsigned char SD_TransmitData(unsigned char data);//послать данные SD-карте и прин€ть ответ
...
unsigned long SD_GetSize(void);//получить объЄм SD-карты в байтах
...
bool SD_ReadBlock(unsigned long BlockAddr,unsigned char *Addr);//считать блок в 512 байт в пам€ть
//----------------------------------------------------------------------------------------------------
//инициализаци€ карты пам€ти
//----------------------------------------------------------------------------------------------------
void SD_Init(void)
{
 ...
 //вывод SPI SS в режиме MASTER сконфигурирован как выход и на SPI не вли€ет
 _delay_ms(100);//пауза, пока карта не включитс€
 unsigned char n;
 //шлЄм не менее 74 импульсов синхронизации при высоком уровне на CS и DI 
 SD_CS_PORT|=(1<<SD_CS);
 _delay_ms(500);
 SD_DI_PORT|=(1<<SD_DI);
 for(n=0;n<250;n++)
 ...
 for(m=0;m<65535;m++)
 {
  ...
  SD_TransmitData(0xff);
  if (answer==0) break;//инициализаци€ успешна
  _delay_us(1);
 }
 if (m==65535)
 {
...
//----------------------------------------------------------------------------------------------------
//послать данные SD-карте и прин€ть ответ
//----------------------------------------------------------------------------------------------------
...
{
 //отправл€ем команду и считаем еЄ CRC7
 ...
 */
 ...
 return(0xff);//ответ не прин€т
}
//----------------------------------------------------------------------------------------------------
//получить объЄм SD-карты в байтах
//----------------------------------------------------------------------------------------------------
unsigned long SD_GetSize(void)
{
 unsigned short n;
 if (SD_SendCommandR1(0x49,0x00,0x00,0x00,0x00)==0xff) return(0xffff);//ответ не прин€т
...
//----------------------------------------------------------------------------------------------------
//считать блок в 512 байт в пам€ть
//----------------------------------------------------------------------------------------------------
